...
def monomers():
    Monomer('R',['b','state'], {'state': ['I','A']}) # Receptor
    Monomer('L', ['b']) # Ligand
...
def initials(l=1e4, r=200):
    ...
    Initial(R(b=None, state='I')**CM, R_0)
    return

def observables():
    alias_model_components()
    Observable('L_free',L(b=None)**EXTRA) # Free ligand
    Observable('LR', L(b=1)**EXTRA % R(b=1)**CM) # total ligand-receptor complex
    Observable('LR_I', L(b=1)**EXTRA % R(b=1, state='I')**CM) # ligand-inactive receptor complex
    Observable('LR_A', L(b=1)**EXTRA % R(b=1, state='A')**CM) # ligand-active receptor complex
...
def single_state_activation(kd=100, kf=1e-3):
    ...
    alias_model_components()
    LR_A = L(b=1)**EXTRA % R(b=1, state='A')**CM # Alias the active receptor complex
    Rule(L(b=None)**EXTRA + R(b=None, state='I') | LR_A, kf_L_bind_R, kr_L_bind_R)
    return
...
def two_state_activation(kd = 100, kf=1e-3, kfa=1e-1, kra=1):
    ...
    Expression('kr_L_bind_R', Kd_LR*kf_L_bind_R)
    alias_model_components()
    LR_I = L(b=1)**EXTRA % R(b=1, state='I')**CM # Alias the inactive receptor complex
    LR_A = L(b=1)**EXTRA % R(b=1, state='A')**CM # Alias the active receptor complex
    Rule(L(b=None)**EXTRA + R(b=None, state='I') | LR_I, kf_L_bind_R, kr_L_bind_R)
...
def reversible_two_state_activation(kd_i=100, kf_i=1e-3,
    ...
    alias_model_components()
    R_I = R(b=1, state='I')**CM # Alias the inactive receptor
    R_A = R(b=1, state='A')**CM # Alias the active receptor
    LR_I = L(b=1)**EXTRA % R(b=1, state='I')**CM # Alias the inactive receptor complex
    LR_A = L(b=1)**EXTRA % R(b=1, state='A')**CM # Alias the active receptor complex
    Rule(L(b=None)**EXTRA + R(b=None, state='I') | LR_I, kf_L_bind_R_I, kr_L_bind_R_I)
    Rule(L(b=None)**EXTRA + R(b=None, state='A') | LR_A, kf_L_bind_R_A, kr_L_bind_R_A)
    Rule(LR_I | LR_A, kf_R_I_to_A, kr_R_I_to_A)
